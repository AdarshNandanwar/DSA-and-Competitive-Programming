...
int solve(int * arr, int n, int open, int close, int** dp){
    if(n == 0){
        if(open == close){
            return 1;
        } else {
    ...
    int ans = 0;
    if(arr[0] == 1){
        ans += solve(arr+1, n-1, open+1, close, dp);
    } else {
        int option1 = solve(arr+1, n-1, open+1, close, dp);
        int option2 = solve(arr+1, n-1, open, close+1, dp);
        ans += (option1 + option2);
    }
    dp[n][open] = ans;
    return ans;
