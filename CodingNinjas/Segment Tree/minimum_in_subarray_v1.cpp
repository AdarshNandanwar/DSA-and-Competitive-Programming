...
int min_in_subarray_2(int * tree, int tree_node, int start, int end, int left, int right){
    if(start == end){
        int ans = tree[tree_node];
        return ans;
    }
    int mid = (start+end)/2, option1, option2, ans;
    if(mid >= right){
        ans = min_in_subarray_2(tree, 2*tree_node, start, mid, left, right);
    } else if(mid < left){
        ans = min_in_subarray_2(tree, 2*tree_node+1, mid+1, end, left, right);
    } else {
        option1 = min_in_subarray_2(tree, 2*tree_node, start, mid, left, right);
        option2 = min_in_subarray_2(tree, 2*tree_node+1, mid+1, end, left, right);
