...
int find_strength(int ** input, int r, int c, int ** dp){
    dp[r-1][c-1] = 0;
    for (int i = r-2; i >= 0; i--)
    {
    ...
    for (int i = c-2; i >= 0; i--)
    {
        dp[r-1][i] = max(dp[r-1][i+1] - input[r-1][i+1], 1);
    }
    for (int i = r-2; i >= 0; i--)
    {
        for (int j = c-2; j >= 0; j--)
        {
            dp[i][j] = max(min(dp[i+1][j] - input[i+1][j], dp[i][j+1] - input[i][j+1]), 1);
        }
    }    
    return dp[0][0];
}
...
int main()
{
    ...
    while(t--){
        ...
        cout<<find_strength(input, r, c, dp)<<endl;
