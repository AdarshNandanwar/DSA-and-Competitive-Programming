...
int Solution::solve(vector<vector<int> > &A) {
    ...
    int n = A.size();
    if(n == 0) return 0;
    int m = A[0].size(), count = 0, sum;
    
    vector<vector<int>> suffixSum(n+1, vector<int>(m+1, 0));
    for(int i = n-1; i>=0; i--){
        for(int j = m-1; j>=0; j--){
            suffixSum[i][j] = A[i][j]-suffixSum[i+1][j+1]+suffixSum[i+1][j]+suffixSum[i][j+1];
        }
    }
    
    // for(auto i:suffixSum){for(auto j:i) cout<<j<<" "; cout<<endl;}
